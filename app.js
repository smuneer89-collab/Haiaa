...
async function saveDevDrafts(){ try{ await storage.set('devDrafts',JSON.stringify(devDrafts)); }catch(e){ toast('تعذر حفظ تعديلات الموقع'); } cloudPush('devDrafts',devDrafts); }
...
/* ═══════════ تقييم الموسم العام ═══════════ */
function seasonEvalPageURL(sessionId){
  const base=location.origin + location.pathname.replace(/[^/]*$/, '');
  return base+'season-evaluation.html?s='+sessionId;
}
...
function renderDevDrafts(root){
  ...
  const oldArea=item?(item.area||item.category||'مكان آخر'):'مكان آخر';
  const form=item?devFormShell(item.id?'تعديل طلب التعديل':'إضافة تعديل في الموقع',`<div class="field full"><label>عنوان التعديل <span class="opt">اختياري</span></label><input id="ddTitle" value="${escapeHtml(item.title||'')}" oninput="scheduleDraftAutosave()" placeholder="مثلاً: تعديل صفحة ملف العضو"></div><div class="field"><label>مكان التعديل *</label><select id="ddArea" onchange="scheduleDraftAutosave()">${devOpts(DEV_SITE_AREAS,DEV_SITE_AREAS.includes(oldArea)?oldArea:'مكان آخر')}</select></div><div class="field"><label>أهمية التعديل</label><select id="ddImportance" onchange="scheduleDraftAutosave()">${devOpts(DEV_PRIORITIES,item.importance||'عادية')}</select></div><div class="field full"><label>المشكلة أو التعديل المطلوب *</label><textarea id="ddProblem" rows="7" oninput="scheduleDraftAutosave()" placeholder="اكتب المشكلة وما الذي تريد تعديله بالتفصيل…">${escapeHtml(oldProblem)}</textarea></div><div class="field full"><label>أهمية التعديل وسببه <span class="opt">اختياري</span></label><textarea id="ddReason" rows="4" oninput="scheduleDraftAutosave()" placeholder="لماذا نحتاج هذا التعديل؟ وما فائدته؟">${escapeHtml(item.reason||'')}</textarea></div><div class="field full"><label>ملاحظات إضافية <span class="opt">اختياري</span></label><textarea id="ddNotes" rows="4" oninput="scheduleDraftAutosave()">${escapeHtml(item.notes||'')}</textarea></div><div class="field"><label>رابط ملف أو صورة</label><input id="ddLink" type="url" value="${escapeHtml(item.link||'')}" oninput="scheduleDraftAutosave()" placeholder="Google Drive أو أي رابط"></div><div class="field"><label><input id="ddPinned" type="checkbox" ${item.pinned?'checked':''} onchange="scheduleDraftAutosave()"> تثبيت التعديل في الأعلى</label></div><div class="field full"><div class="dev-meta" id="draftSaveState">${item.id?'آخر حفظ: '+devFmt(item.updatedAt):'يبدأ الحفظ التلقائي بعد كتابة المشكلة'}</div></div>`,'saveDevDraft'):`<button class="btn btn-primary" onclick="devNew()">+ إضافة تعديل في الموقع</button>`;
  const aiActions=item?`<div class="dev-actions"><button class="btn btn-ghost" onclick="copyDevDraftForAI()">نسخ طلب التعديل للذكاء الاصطناعي</button><button class="btn btn-ghost" onclick="openDevDraftInChatGPT()">نسخ الطلب وفتح ChatGPT</button></div>`:'';
...
function scheduleDraftAutosave(){clearTimeout(devDraftSaveTimer);const s=$('#draftSaveState');if(s)s.textContent='بانتظار الحفظ التلقائي…';devDraftSaveTimer=setTimeout(()=>saveDevDraft(true),1800);}
function devDraftAIText(d){return `أريد صياغة طلب تعديل واضح واحترافي لموقعي، مع المحافظة على جميع الوظائف الحالية وعدم حذف أو تغيير أي شيء غير مطلوب.\n\nعنوان التعديل: ${d.title||'غير محدد'}\nمكان التعديل: ${d.area||d.category||'غير محدد'}\nالمشكلة أو التعديل المطلوب:\n${d.problem||d.body||'—'}\n\nأهمية التعديل: ${d.importance||'عادية'}\nسبب التعديل وأهميته:\n${d.reason||'—'}\n\nملاحظات إضافية:\n${d.notes||'—'}\n${d.link?'\nرابط مرفق: '+d.link:''}\n\nأعد صياغة هذا الطلب بصورة مرتبة ودقيقة، وحدد المطلوب بوضوح، ولا تضف وظائف لم أطلبها.`;}
async function copyDevDraftForAI(){const d=draftFormData();if(!d.problem){toast('اكتب المشكلة أولاً');return false;}await copyToClipboard(devDraftAIText(d));toast('تم نسخ طلب التعديل');return true;}
async function openDevDraftInChatGPT(){if(await copyDevDraftForAI())window.open('https://chatgpt.com/','_blank','noopener');}
async function copyStoredDevDraftForAI(id){const d=devDrafts.find(x=>x.id===id);if(!d)return;await copyToClipboard(devDraftAIText(d));toast('تم نسخ طلب التعديل');}
